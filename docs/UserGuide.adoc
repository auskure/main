...
==== Downloading all notesList from a selected module from IVLE : `downloadAllNotes`

Downloads the your notesList from IVLE, and stores them in the folder "notesList" +
...
* Automatically accesses the your IVLE account using Google Chrome, and downloads all notesList and resources
...
* If the process is successful, the result window of NSync will display the exact path to your newly downloaded notesList.
****
...
==== Display all notesList from a selected module from IVLE : `downloadSelectNotes` without prefix `"file/"`
Displays all the available notesList of a selected +
...
* All available notesList from the selected module would be sourced and displayed
...
==== Downloading selective notesList from a selected module from IVLE : `downloadSelectNotes` with prefix `"file/"`
Download a selective number of notesList from IVLE, and stores them in the folder "notesList".
...
* The files would be downloaded one by one from IVLE to your "notesList" folder.
...
==== Show all downloaded notesList: `showNotes`
Displays all the downloaded notesList in the UI +
Format: `showNotes`

[NOTE]
the files are displayed in the order that it is in your directory

* It recursively searches your `notesList` folder and displays all the files available
...
// tag::clearNotes[]
==== Clearing all downloaded notesList : `clearNotes`

Clears all downloaded notesList. +
Format: `clearNotes`
// end::clearNotes[]
// tag::deleteSelectNotes[]
==== Deleting notesList from one or more selected modules : `deleteSelectNotes`

Deletes the notesList that belong to the specified module, from storage +
Format: `deleteSelectNotes [ENROLLED MODULE]1..`

****
* Deletes notesList of the `ENROLLED MODULE`.
* The notesList of the `ENROLLED MODULE` would be deleted, if they were downloaded using the command
...
Deletes the notesList belonging to the module CS2100.
* `deleteSelectNotes CS2105 CS2106` +
Deletes the notesList belongs to the modules CS2105 and CS2106.
* `deleteSelectNotes CS` +
Deletes the notesList belongs to modules that have "CS" in their names
* `deleteSelectNotes PL3232` +
Will not delete anything if "PL3232" does not exist as your downloaded notesList.
* `deleteSelectNotes PL3232 CS2106` +
Will not delete anything if "PL3232" does not exist as your downloaded notesList, however notesList
...
* *Delete selected notesList* : `deleteSelectedNotes [ENROLLED MODULE]1.." +
e.g. `deleteSelectedNotes CS2105 CS2106`

* *Download All Notes* : `downloadAllNotes user/e1234567 pass/password1 mod/CS2113`

* *Download Selected Notes (selected notesList)* : `downloadSelectNotes user/e1234567 pass/password1 mod/CS2113 file/1,2,3...`

* *Download Selected Notes (show all notesList)* : `downloadSelectNotes user/e1234567 pass/password1 mod/CS2113`

* *Show notesList* : `showNotes`
