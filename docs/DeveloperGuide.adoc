...
The delete notesList mechanism is facilitated by `NotesDownloaded`.
It is a part of `ModelManager`, storing the names of the modules of notesList
downloaded.
Additionally, it implements the following operations:

* `NotesDownloaded#setNotes(Set<String> notesList)` -- Replaces the contents of the notesList list with {@code notesList}.
* `NotesDownloaded#addNotes(String n)` --  Adds new notesList {@code n} to the NotesDownloaded
...
Given below is an example usage scenario and how the notesList manipulation mechanism behaves at each step.

Step 1. The user launches the application for the first time. The `NotesDownloaded` will be initialized with the
initial notesList state, which is empty.

image::NotesManipulationStartingStateTreeDiagram.png[width="800"]

Step 2. The user executes `downloadAllNotes user/e0123456 pass/password1 mod/CS2101` command to download all notesList
for the module CS2101.  The `downloadAllNotes` command calls `Model#addNotes(String, String)`, module code of the newly
 downloaded notesList to be saved in notesList
...
|`* * *` |user |download the notesList of a specific module in one command | avoid downloading unnecessary notesList

|`* * *` |lazy IVLE user |download all my notesList in one command | avoid the cumbersome experience of visiting IVLE

|`* * *` |user |view the notesList already downloaded | keep track of the notesList that I already have

|`* * *` |user |delete all downloaded notesList | free up some space on my local drive

|`* * *` |user |view my enrolled modules | store my notesList according to the modules I am enrolled in
...
|`* *` |user |delete the notesList of a specified module |keep my local drive organized
