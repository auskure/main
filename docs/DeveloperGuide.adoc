...
image::ChangeTimeslot1.PNG[width="800"]
...
image::ChangeTimeslot2.PNG[width="800"]
...
image::MergeCommand1.PNG[width="200"]
...
image::MergeCommand2.PNG[width="400"]

Step 3. The `mergeTimetable` function is called on each `Person` in `personsToMerge` and the element after it. The merge
...
The `Name`s of each `Person` are appended together and gets saved in the `Address` of the merged
...
Step 5. When `personsToMerge` is fully iterated through, the last
...
image::MergeCommand6.PNG[width="800"]
...
image::UpdateCommand1.PNG[width="800"]
...
image::UpdateCommand2.PNG[width="800"]
...
image::UpdateCommand3.PNG[width="800"]

The following sequence diagram shows how the `update` function works.

image::UpdateCommand4.PNG[width="800"]
